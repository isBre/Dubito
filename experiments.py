...
def save_stats(stats : Dict, data_name : str = "all_games.yaml"):
...
for i in tqdm(range(N_EXPERIMENTS), desc = "Playing Games ..."):
    ...
    results, infos = dubito(all_players)
    for idx, p in enumerate(all_players):

        result = 'losses'
        if p in results['winners']:
            result = 'wins'
...
for p_alg in players_alg:
    ...
    for key in keys:
        final_infos[p_alg][key]['avg_cards'] = final_infos[p_alg][key]['avg_cards'] / final_infos[p.__class__.__name__][key]['games']
