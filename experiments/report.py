...
def generate_html_report(final_infos: dict, config: dict, output_path: str = 'report.html') -> None:
    ...
    win_rates  = [win_rate(final_infos[b]) for b in players]
    ...
    fig_cards.update_layout(
        barmode='group', title='Average Cards in Hand', yaxis_title='Avg cards',
        plot_bgcolor='white', paper_bgcolor='white',
        font_family='Inter, sans-serif', margin=dict(t=60, b=80),
    ...
    def _heatmap_fig(position: str, title: str) -> go.Figure:
        ...
        fig.update_layout(
            ...
            plot_bgcolor='white', paper_bgcolor='white',
            font_family='Inter, sans-serif', margin=dict(t=60, b=100, l=100),
        )
    ...
    def _scatter(x_key: str, y_key: str, x_label: str, y_label: str, title: str) -> go.Figure:
        ...
        fig.update_layout(
            ...
            plot_bgcolor='white', paper_bgcolor='white',
            font_family='Inter, sans-serif', margin=dict(t=60, b=60), height=420,
        )
        return fig
    ...
    def _neighbor_fig(bot: str, position: str) -> go.Figure:
        ...
        fig.update_layout(
            ...
            plot_bgcolor='white', paper_bgcolor='white',
            font_family='Inter, sans-serif', margin=dict(t=50, b=70), height=300,
        )
    ...
    fig_win_space.update_layout(
        ...
        xaxis_title='Hard Win % (1st place)', yaxis_title='Soft Win % (2nd to n−2)',
        xaxis_tickformat='.0%', yaxis_tickformat='.0%',
        plot_bgcolor='white', paper_bgcolor='white',
        font_family='Inter, sans-serif', margin=dict(t=60, b=60), height=480,
    )
    ...
    def _table_rows(sorted_bots: list, rate_fn, base: float, bucket: str) -> str:
        ...
        <tr>
          ...
          <td class="text-end" style="color:{'#198754' if rate >= base else '#dc3545'};">{(rate - base):+.1%}</td>
        ...
        <div class="row g-3 mb-3">
          ...
          <div class="col-6 col-md-2">
            <div class="stat-card">
              ...
              <div class="progress mt-1" style="height:4px;"><div class="progress-bar bg-secondary" style="width:{int(swr*100)}%;"></div></div>
            </div>
          </div>
          <div class="col-6 col-md-2">
            <div class="stat-card">
...
<head>
  ...
  <style>
    ...
    .stat-card {{ background:#fff; border:1px solid #e9ecef; border-radius:8px; padding:12px 16px; text-align:center; }}
    ...
    .section-title {{ font-size:1.4rem; font-weight:700; border-left:4px solid #4C78A8; padding-left:12px; margin-bottom:1rem; }}
...
<div class="container-lg py-4">
  ...
  <section id="overview" class="mb-5">
    ...
    <div class="table-responsive mb-5">
      <table class="table table-hover table-bordered align-middle mb-0 bg-white shadow-sm">
        <thead><tr>
          ...
          <th class="text-end">Hard Win %</th><th class="text-end">Avg cards (hard win)</th>
          <th class="text-end">vs Baseline</th><th class="text-end">Avg cards on loss</th>
        </tr></thead>
    ...
    <p class="text-muted small mb-2">A <strong>soft win</strong> means the bot emptied their hand while ≥3 players were still active, but did not finish 1st.
      Baseline ≈ {soft_base:.1%} ((avg players − 3) / avg players).</p>
    <div class="table-responsive">
      <table class="table table-hover table-bordered align-middle mb-0 bg-white shadow-sm">
        <thead><tr>
          <th class="text-center">#</th><th>Bot</th>
          <th class="text-end">Total games</th><th class="text-end">Soft Wins</th>
          <th class="text-end">Soft Win %</th><th class="text-end">Avg cards (soft win)</th>
          <th class="text-end">vs Baseline</th><th class="text-end">Avg cards on loss</th>
        </tr></thead>
  ...
  <section id="global" class="mb-5">
    ...
    <div class="row g-4">
      <div class="col-12"><div class="chart-card">{_div(fig_wr, height='440px')}</div></div>
      <div class="col-12"><div class="chart-card">{_div(fig_cards, height='440px')}</div></div>
      ...
      <div class="col-12"><div class="chart-card">{_div(fig_pos, height='440px')}</div></div>
      <div class="col-12">
  ...
  <section id="scatter" class="mb-5">
    ...
    <div class="row g-4">
      <div class="col-12 col-xl-6"><div class="chart-card">{_div(fig_sc_style,   height='440px')}</div></div>
      <div class="col-12 col-xl-6"><div class="chart-card">{_div(fig_sc_agg_wr,  height='440px')}</div></div>
      <div class="col-12 col-xl-6"><div class="chart-card">{_div(fig_sc_quality, height='440px')}</div></div>
      <div class="col-12 col-xl-6"><div class="chart-card">{_div(fig_sc_pos_wr,  height='440px')}</div></div>
    </div>
  </section>
